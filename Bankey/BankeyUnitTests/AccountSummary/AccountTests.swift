...
class AccountTests: XCTestCase {
    ...
    func testCanParse() throws {
        ...
        XCTAssertEqual(account1.createdDateTime.dayMonthYearString, "21.06.2010")
        
        let account2 = results[1]
        XCTAssertEqual(account2.id, "2")
        XCTAssertEqual(account2.type, AccountType.Banking)
        XCTAssertEqual(account2.name, "No-Fee All-In Chequing")
        XCTAssertEqual(account2.amount,17562.44)
        XCTAssertEqual(account2.createdDateTime.dayMonthYearString, "21.06.2011")
    }
}
